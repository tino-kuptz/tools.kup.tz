<script setup>
import { parseMsg } from '~/utils/msgParser.js';

const { t } = useI18n();

useCustomI18nHead({
  title: t('pages.formatting.msgViewer.title'),
  meta: [
    {
      description: t('pages.formatting.msgViewer.description'),
    },
  ],
});
...
function onFileChange(event) {
  ...
  reader.onload = (e) => {
    ...
    if (!(raw instanceof ArrayBuffer)) {
      parseError.value = t('pages.formatting.msgViewer.invalidMsg');
      return;
    }
    const result = parseMsg(raw);
    if (result.error) {
  ...
  reader.onerror = () => {
    parseError.value = t('pages.formatting.msgViewer.parseError');
  };
...
<template>
  <div>
    <h2 class="mb-2">
      {{ t('pages.formatting.msgViewer.heading') }}
    </h2>
      ...
      <VCardItem>
        <div>
          ...
          <div class="text-h6 mb-1">
            {{ t('pages.formatting.msgViewer.descriptionText') }}
          </div>
          <div class="text-h6 mb-1">
            {{ t('pages.formatting.msgViewer.descriptionPrivacy') }}
          </div>
        </div>
      </VCardItem>
    </VCard>
    ...
    <VCard class="mb-4">
      <VCardText>
        <h3 class="mb-3">
          {{ t('pages.formatting.msgViewer.selectFile') }}
        </h3>
        <input
          type="file"
          accept=".msg"
      ...
      <VCard>
        <VCardText>
          <span class="text-medium-emphasis">
            {{ t('pages.formatting.msgViewer.noFileSelected') }}
          </span>
        </VCardText>
      </VCard>
    </template>
    ...
    <template v-else-if="parsed">
          ...
          <VWindowItem value="text/html">
            <VCardText v-if="htmlPart">
              <div class="mb-4">
                <VCheckbox
                  v-model="formatHtml"
                  :label="t('pages.formatting.msgViewer.formatHtml')"
                  density="compact"
                  hide-details
                />
                <span v-if="formatHtml" class="text-caption text-medium-emphasis">
                  {{ t('pages.formatting.msgViewer.formatHtmlNote') }}
                </span>
              </div>
              <iframe
                v-if="formatHtml"
                sandbox="allow-same-origin"
                title="HTML Nachricht"
                class="eml-body-iframe"
      ...
      >
        ...
        <VCardText>
          ...
          <a
            v-if="part.blob"
            :href="getAttachmentUrl(part.blob)"
            :download="part.filename || `part-${idx}`"
            class="v-btn v-btn-size-small v-btn-variant-elevated v-btn-color-primary"
          >
            {{ t('pages.formatting.msgViewer.downloadAttachment') }}
          </a>
        </VCardText>
      </VCard>
      ...
      <VCard v-if="parsed.attachments?.length" class="mb-4">
        <VCardTitle>
          {{ t('pages.formatting.msgViewer.attachments') }} ({{ parsed.attachments.length }})
        </VCardTitle>
        <VCardText>
          <ul class="eml-attachments list-unstyled">
            <li v-for="(att, idx) in parsed.attachments" :key="idx" class="d-flex align-center gap-2 py-2">
              ...
              <a
                :href="getAttachmentUrl(att.blob)"
        ...
        <VExpansionPanels>
          <VExpansionPanel>
            <VExpansionPanelTitle>
              {{ t('pages.formatting.msgViewer.allHeaders') }}
            </VExpansionPanelTitle>
            <VExpansionPanelText>
